...
// queue<string> PARAN_STRINGS; //holds 
...
void precedenceParser(string &input);
...
int main(){
  ...
  while (run){
    ...
    commentCheck(input); 
    pair<bool, bool> results = checkInput(input, '(', ')'); //checks parantheses
    if (!results.first){
      cout << "You have entered the wrong amount of parantheses. Try again." << endl << endl;
      continue;
    }
    results = checkInput(input, '[' , ']'); //checks brackets
    if (!results.first){
